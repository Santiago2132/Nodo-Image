...
class GestorNodos:
    ...
    # En nodo.py, reemplazar el método procesar_xml_imagenes en la clase GestorNodos
...
def procesar_xml_imagenes(self, xml_content, aplicar_transformaciones=True):
    ...
    try:
        ...
        xmls_temporales = []
        nodos_procesados = []
        formatos_individuales = []  # NUEVO: guardar formato de cada imagen
        
        # Procesar cada imagen
        for i, imagen_elem in enumerate(imagenes):
            datos_b64 = imagen_elem.text
            transformaciones = imagen_elem.get('transformaciones', '')
            formato = imagen_elem.get('formato', 'JPEG').upper()  # Respetar formato individual
            
            formatos_individuales.append(formato)  # NUEVO: almacenar
                ...
                if aplicar_transformaciones and transformaciones:
                    ...
                    for trans in trans_list:
                        if 'escala_grises' in trans:
                            ...
                            formato_convertir = trans.split('_')[-1].upper()
                            nodo.convertir_formato(formato_convertir)
                            formatos_individuales[i] = formato_convertir  # NUEVO: actualizar
                        elif 'recortar' in trans:
                            pass
                nodos_procesados.append((nodo, formato))  # NUEVO: tupla con formato
        ...
        # Fusionar todos los nodos procesados
        procesadas = 0
        ...
        for j, (nodo, formato_original) in enumerate(nodos_procesados):
            try:
                ...
                # NUEVO: usar formato individual de cada imagen
                formato_final = formatos_individuales[j] if j < len(formatos_individuales) else formato_original
                calidad = 90 if formato_final == 'PNG' else 80
                
                nodo.generar_xml_optimizado(temp_output, formato_final, calidad)
